...
|---|---|---|---|---|---|---|---|
| World Bank (`WB_WDI`) | PARTIALLY VERIFIED — payload shape matches WB's own documented API; ID-format bug fixed this phase | VERIFIED — sdmx1's `TestWB_WDI` exercises this exact key format live | PARTIALLY VERIFIED — `ref_area` accepted as an opaque code; discovery doesn't enumerate a country codelist | PARTIALLY VERIFIED — `startPeriod`/`endPeriod` pass through untouched; label parsing relies on generic `sdmx1` behavior, not WDI-specific confirmation | NOT VERIFIED (observations) / PARTIALLY VERIFIED (catalog metadata) — discovery captures `unit` from the API payload, but `SDMXProvider._to_series_result` never copies it onto `SeriesResult` (Phase F) | VERIFIED — `Attribution` always carries `source_id`/`dataset_id`/`retrieved_at`/`source_url` | NOT VERIFIED |
...
| Eurostat (`ESTAT_NAMA_10_GDP`) | VERIFIED — `sdmx1`'s `TestESTAT.test_ss_data` walks the exact dataflow→external-reference→resolve path live | VERIFIED — same test, key format `"A.CP_MEUR.B1GQ.LU"` | VERIFIED — ref-area codelist enumerated from the same resolved DSD | PARTIALLY VERIFIED — same generic-`sdmx1`-behavior caveat | PARTIALLY VERIFIED — unit is structurally correct and fixed (`CP_MEUR`, current-price million EUR) for every entry in this scoped dataflow, but the human-readable value is never written into `IndicatorEntry.unit`/`SeriesResult.unit` (Phase F) | VERIFIED | NOT VERIFIED |
